...
module "consul_cluster" {
  ...
  resource_group_id = data.ibm_resource_group.project.id
  name              = "${var.project_name}-${data.ibm_is_zones.regional_zones.zones[count.index]}"
  zone              = data.ibm_is_zones.regional_zones.zones[0]
  security_groups   = [module.bastion.bastion_maintenance_group_id, module.security.consul_security_group]
  tags              = concat(var.tags, ["zone:${data.ibm_is_zones.regional_zones.zones[count.index]}", "region:${var.region}", "project:${var.project_name}"])
